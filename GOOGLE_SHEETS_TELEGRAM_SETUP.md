...
У файлі `сайт/maxbat.ua/index.html` вставте посилання у рядок:
```javascript
...
## 👥 Як надсилати сповіщення у спільну групу менеджерів

Якщо потрібно, щоб сповіщення отримував не один користувач, а ціла команда менеджерів:

1. **Дозвольте боту входити в групи:**
   - Відкрийте `@BotFather` ➔ `/mybots` ➔ оберіть вашого бота.
   - Натисніть **Bot Settings** ➔ **Allow Groups?** ➔ **Turn groups on**.
2. **Створіть групу в Telegram** (або відкрийте існуючу) та додайте вашого бота.
3. **Зробіть бота Адміністратором** групи (налаштування групи ➔ *Керування ➔ Адміністратори*).
4. **Дізнайтеся ID групи:**
   - Додайте бота `@myidbot` у групу та напишіть команду `/getgroupid@myidbot`.
   - Скопіюйте ID (обов'язково зі знаком мінус, наприклад `-5453960944` або `-100...`).
   - Видаліть `@myidbot`.
5. **Вкажіть цей ID в Apps Script:**
   ```javascript
   const TELEGRAM_CHAT_ID = "-5453960944"; // обов'язково в лапках та з мінусом!
   ```
6. Збережіть скрипт (💾). Тепер усі менеджери в групі миттєво отримуватимуть заявки!
